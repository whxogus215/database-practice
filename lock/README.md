...
3. [Mission 4: Named Lock With Multi Connection Pool](./MISSION_4.md)
...
**핵심 문제**: 이벤트 신청 상황에서 발생하는 동시성 문제  
**해결 전략**:
- **비관적 락** : 하나의 트랜잭션만 레코드에 접근하도록 락을 선점하는 방식, 충돌이 잦은 상황에서 효과적
- **낙관적 락** : 버전 필드 비교를 통해 불일치하면 재시도하는 방식, 충돌이 잦은 상황에서는 재시도로 인해 부적합

### 3. 네임드 락: 논리적인 행위에 잠금을 거는 방법
**핵심 문제**: 이벤트 신청 상황에서 발생하는 동시성 문제  
**해결 전략**:
- **네임드 락** : 특정 문자열(ex. "event_1")을 선점한 트랜잭션만 수행하도록 제어하는 방법, 여러 테이블을 사용하는 로직을 논리적인 행위로 락을 걸 때 사용하기에 적합
  - 트랜잭션 전파 속성 및 커넥션 풀 크기 조정으로 네임드 락 사용 시 발생하는 주요 문제 해결

### 4. 커넥션 풀 분리 : 네임드 락의 커넥션 고갈 및 데드락 문제를 해결하는 방법
**핵심 문제**: 네임드 락을 사용할 경우, 정합성을 위해 부모/자식 트랜잭션이 독립된 커넥션을 사용하는 상황에서 발생하는 커넥션 고갈 문제  
**해결 전략**:
- 데이터소스(`DataSource`) 다중화를 위한 설정 분리 및 수동 빈 등록
- 네임드 락과 비즈니스 전용 커넥션 풀을 분리시키고 `JPA` or `JdbcTemplate`을 사용해 네임드 락 리소스 획득 or 해제
---
## 📖 참고 자료
